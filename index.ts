...
let myBalance = 50000;
let myPin = 5147;
console.log(`Wellcom to my ATM machine`);
...
async function main(): Promise<void> {
  ...
  do {
    ...
    if (answerAction.operation === "withdrow") {
      ...
      console.log(
        chalk.cyan(`You exited from ATM... Good bye have a nice day`)
      );
      exit = true;
    }
  } while (!exit);
  ...
  //withdrow function...
  async function withDrow(): Promise<void> {
    ...
    if (ansewrsAmount.amount > myBalance && myBalance > 0) {
      console.log(
        chalk.redBright(`You entered an amount  higher than your balance`)
      );
    } else if (myBalance <= 0) {
      ...
      console.log(
        chalk.greenBright(
          `You withdrow ${ansewrsAmount.amount} from your balance`
        )
      );
    }
  }
  ...
  async function fastCash(): Promise<void> {
    ...
    if (answerFastcash.fastcash === "1000") {
      ...
      console.log(
        chalk.greenBright(
          `You withdrow ${answerFastcash.fastcash} from your balance`
        )
      );
